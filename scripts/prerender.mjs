...
async function run() {
  ...
  const staticPages = [
    ...
    ["/live-proxies-review", "Live Proxies Review 2026: Pricing, Speed & Real Test Results", "We tested Live Proxies' private residential proxies for pricing, speed, uptime & bot/app integrations. Real results, who it's best for.", `<h1 style="font-size:2rem;font-weight:800;color:#1e3a5f;margin-bottom:.5rem">Live Proxies Review 2026</h1><p style="font-size:1.1rem;margin-bottom:1.5rem">Live Proxies sells private, dedicated residential proxies rather than access to a shared pool, serving both individual (B2C) and enterprise (B2B) customers. Published plans include US state-level IP targeting, sticky rotation, bandwidth rollover, IP whitelisting, dashboard access, API access, 24/7 support, a dedicated manager and no concurrency limit. The B2B network is marketed at 10M+ IPs with custom integrations.</p><h2 style="font-size:1.4rem;font-weight:700;color:#1e3a5f;margin-top:2rem">Live Proxies Pricing 2026</h2><ul style="margin:1rem 0;padding-left:1.5rem"><li>Entry plan tier - metered from $45.00/plan</li><li>Entry plan tier - unmetered from $90.00/plan</li><li>Higher plan tier - metered from $65.00/plan</li><li>Higher plan tier - unmetered from $150.00/plan</li></ul><h2 style="font-size:1.4rem;font-weight:700;color:#1e3a5f;margin-top:2rem">Top Use Cases</h2><p style="margin-bottom:1rem">eCommerce price tracking, social media management (SMM), market research, web scraping without blocks, travel fare aggregation, brand protection, SEO and SERP tracking, and AdTech ad verification. Live Proxies also markets direct compatibility with browser-automation tooling such as Selenium.</p><h2 style="font-size:1.4rem;font-weight:700;color:#1e3a5f;margin-top:2rem">Metered vs Unmetered</h2><p style="margin-bottom:1rem">Metered plans bill against a bandwidth allowance with rollover on unused GB; unmetered plans remove the meter for roughly double the price. Metered suits bursty or unproven workloads, unmetered suits always-on collection where a stalled crawl costs more than the price difference.</p><h2 style="font-size:1.4rem;font-weight:700;color:#1e3a5f;margin-top:2rem">Compare Live Proxies To</h2><ul style="margin:1rem 0;padding-left:1.5rem">${providers.filter(p => ["bright-data","oxylabs","iproyal"].includes(p.slug)).map(p => `<li><a href="${SITE}/reviews/${p.slug}" style="color:#2563eb">${p.name}</a> - ${p.tagline}</li>`).join("")}</ul>`],
    ["/proxys-io-review", "PROXYS.IO Review 2026: Pricing, Coverage & 35% Discount Code", "PROXYS.IO residential rotating proxies from $1.50/GB across 100M+ IPs in 195 countries, plus IPv4, IPv6, ISP and dedicated mobile plans. Code HEAT_RES gives 35% off.", `<h1 style="font-size:2rem;font-weight:800;color:#1e3a5f;margin-bottom:.5rem">PROXYS.IO Review 2026</h1><p style="font-size:1.1rem;margin-bottom:1.5rem">PROXYS.IO is operated by ONLINE CONNECT LTD (company number 15419378, 85 Great Portland Street, First Floor, London, W1W 7LT) and has been trading since 2016. Two products are billed by traffic - residential rotating and mobile rotating - while every other proxy type is billed per address per month. HTTP, HTTPS and SOCKS5 are issued together on every plan, the minimum static rental is one month and there is a 24 hour refund window. Reader code HEAT_RES gives 35% off residential pay-per-traffic plans.</p><h2 style="font-size:1.4rem;font-weight:700;color:#1e3a5f;margin-top:2rem">PROXYS.IO Pricing 2026</h2><ul style="margin:1rem 0;padding-left:1.5rem"><li>Residential rotating - from $1.50/GB - 30, 60 and 90 day plans, no IP or session cap</li><li>Mobile rotating (3G/4G/LTE/5G) - pay as you go over 30, 60 and 90 days - rate calculated by a slider in the cart, not published as a single figure</li><li>Individual IPv6 - from $0.13 per address per month</li><li>Dynamic IP - from $0.27 per address per month</li><li>Shared IPv4 (up to 3 users) - from $0.67 per address per month</li><li>Foreign individual IPv4 - from $1.47 per address per month</li><li>IPv4 with a Windows OS fingerprint - from $1.87 per address per month</li><li>Premium static residential IPv4 - from $3.60 per address per month</li><li>Dedicated mobile IPv4, unlimited traffic on a private channel - from $56.79 per month</li></ul><h2 style="font-size:1.4rem;font-weight:700;color:#1e3a5f;margin-top:2rem">Pool Size And Coverage</h2><p style="margin-bottom:1rem">Residential: over 100 million ISP addresses across 195 countries, targetable by country, region and city, with up to 100 saved parameter sets per account and rotation on a 2 to 60 minute timer. Mobile: 14 countries with city and carrier selection, including the United States, United Kingdom, France, Spain, Poland, Romania, Bulgaria, Estonia, Moldova, Armenia, Ukraine, Indonesia and Thailand, with roughly 2,000 to 5,000 addresses per country and carrier combination. Static IPv4 is available in Ukraine, the United Kingdom, the United States, Germany, India, Poland, Spain, Belarus, the Netherlands, Kazakhstan, France, Turkey, Romania, Italy, Brazil, Canada, Slovenia, Lithuania, Bangladesh, Argentina, Sweden, Hong Kong, Australia and South Africa.</p><p style="margin-bottom:1rem">Specifications above are provider-reported. We have not independently benchmarked PROXYS.IO speed, uptime or anti-bot success rates.</p><h2 style="font-size:1.4rem;font-weight:700;color:#1e3a5f;margin-top:2rem">Compare PROXYS.IO To</h2><ul style="margin:1rem 0;padding-left:1.5rem">${providers.filter(p => ["iproyal","decodo","proxy-cheap"].includes(p.slug)).map(p => `<li><a href="${SITE}/reviews/${p.slug}" style="color:#2563eb">${p.name}</a> - ${p.tagline}</li>`).join("")}</ul>`],
